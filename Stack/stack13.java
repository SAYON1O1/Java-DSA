...
public class stack13 {
    public static void main(String[] args) {
        Stack<Integer> s= new Stack<>();  // static ArrayList<Integer> list = new ArrayList<>();

                  //     public static void push(int data) {
                  //         list.add(data);   // adding from last [1,2,3,4]
                  //     } 

        s.push(1);   //list.add(data);  
        s.push(2);
        s.push(3); 
            while(!s.isEmpty()){
                System.out.println((s.peek())); //list.get(list.size()-1); 
                s.pop();  //list.remove(list.size()-1);
            }
    }
